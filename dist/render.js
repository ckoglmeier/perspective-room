...
export function buildRoomBundle(room) {
...
export function renderAgentMarkdown(room) {
...
export function renderRoomHtml(room) {
    const company = room.company?.name || room.title;
...
function claimLines(room) {
    const claims = room.claims ?? [];
    if (claims.length === 0)
        return ['- No claim registry included.'];
...
function bundleWarnings(room) {
    const warnings = [];
    const generatedAt = room.updated_at || room.created_at;
    if (!generatedAt)
        warnings.push('room has no created_at or updated_at timestamp');
    return warnings;
}
